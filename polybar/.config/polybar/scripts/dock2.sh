#!/bin/bash
_names="web code opt1 opt2 opt3 opt4 opt5 opt6 opt7 msg music"
IFS=' ' read -r -a names <<< "$_names"
...
for monitor in "${order[@]}"; do
	...
	for desk in $desks; do
		deskid="$(bspc query -D -m $monitor | awk NR==$desk)"
		thisd=$(bspc query -D -d "${names[$desk_cnt]}")
    echo $des_cnt "hi" $deskid "hello" $thisd
		if [[ $deskid -ne $thisd ]]; then
			echo "Swap $deskid and $thisd"
			bspc desktop $deskid -s $thisd
		fi
		desk_cnt=$(($desk_cnt + 1))
	done
...
for monitor in "${order[@]}"; do
	...
	desk=${names[@]:$desk_cnt:$((desk_cnt+$this_desktops-1))}
