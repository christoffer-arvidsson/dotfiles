#+title: Emacs config
#+property: header-args:elisp :tangle ~/.config/emacs/init.el :cache no :results silent :padline no
#+property: header-args:emacs-lisp :tangle ~/.config/emacs/init.el :cache no :results silent :padline no
#+auto_tangle: t
...
The following block is stolen from doom, but handles pressing enter on certain items.
...
#+end_src
** Close minibuffer on lose focus
#+begin_src elisp
...
(use-package citar-org-roam
  :straight (:type git :host nil :repo "https://github.com/emacs-citar/citar-org-roam")
...
(use-package consult
  ...
  (eethern/leader-key-def
    ...
    "b p" '(consult-project-buffer :wk "Find buffer in current project")
))
#+end_src
*** Projectile
#+begin_src elisp
...
(use-package projectile
  ...
  (eethern/leader-key-def
    ...
    "p s" '(+vertico/project-search :wk "Search project")
